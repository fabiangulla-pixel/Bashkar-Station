...
def construir_grafo(
    ...
    for cat in categorias:
        ...
        for ent, arts in indice_global[cat].items():
            ...
            if not G.has_node(ent):
                G.add_node(ent,
                           categoria=cat,
                           color=_COLOR_CAT.get(cat, "#95A5A6"),
                           freq=len(arts))
    ...
    if G.number_of_nodes() > max_nodos:
        ...
        nodos_ord = sorted(G.nodes(data=True),
                           key=lambda x: x[1].get("freq", 0), reverse=True)
...
def metricas_avanzadas(G) -> dict:
    ...
    try:
        ...
        if partition:
            ...
            resultado["comunidades"] = sorted(
                comunidades.items(), key=lambda x: -len(x[1]))
        else:
...
def evolucion_temporal(
    ...
    for i, numero in enumerate(numeros):
        ...
        try:
            G = construir_grafo(indice, categorias=categorias,
            ...
            serie.append({"numero": numero, "nodos": 0, "aristas": 0,
                          "densidad": 0, "top_nodo": "", "top_cent": 0})
