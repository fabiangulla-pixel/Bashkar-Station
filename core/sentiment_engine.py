...
def analizar_corpus_tono(
    ...
    with ThreadPoolExecutor(max_workers=min(workers, max(1, total))) as pool:
        futures = {pool.submit(_analizar_uno, aid, entrada): aid
                   for aid, entrada in articulos.items()}
...
def evolucion_temporal(
    ...
    for periodo in sorted(por_periodo.keys()):
        ...
        total  = sum(conteo.values())
        evolucion[periodo] = {
            tono: round(100 * conteo.get(tono, 0) / total, 1)
            for tono in TONOS
        }
...
def tendencia_tono(
    ...
    if len(valores) < 2:
        return {"periodos": periodos, "valores": valores,
                "pendiente": 0.0, "direccion": "estable"}
...
_LEXICON_EMOCIONES: dict[str, list[str]] = {
    "alegria": [
        "alegre", "feliz", "gozo", "júbilo", "contento", "dichoso", "festivo",
        "triunfo", "celebración", "regocijo", "risueño", "animado", "entusiasta",
        "exultante", "alborozado", "placer", "satisfacción", "bienestar",
        "fiesta", "victoria", "éxito", "brillante", "glorioso",
    ],
    "tristeza": [
        "triste", "dolor", "pena", "luto", "melancolía", "aflicción", "llanto",
        "lágrimas", "desgracia", "duelo", "deplorable", "lamentable", "muerte",
        "fallecimiento", "pérdida", "soledad", "desamparo", "angustia",
        "sufrimiento", "miseria", "penoso", "funesto", "trágico",
    ],
    "miedo": [
        "miedo", "terror", "pánico", "amenaza", "peligro", "alarma", "temor",
        "espanto", "horror", "ansiedad", "nervioso", "inquieto", "intranquilo",
        "amenaza", "riesgo", "pavoroso", "aterrador", "temeroso",
    ],
    "ira": [
        "ira", "rabia", "furia", "cólera", "indignación", "enojo", "protesta",
        "denuncia", "acusación", "ataque", "combate", "lucha", "violencia",
        "conflicto", "escándalo", "controversia", "polémica",
    ],
    "sorpresa": [
        "sorpresa", "asombro", "admiración", "estupor", "maravilla", "increíble",
        "insólito", "extraordinario", "inusitado", "inesperado", "revelación",
        "novedad", "descubrimiento",
    ],
    "confianza": [
        "confianza", "seguridad", "fe", "esperanza", "progreso", "prosperidad",
        "desarrollo", "avance", "modernidad", "paz", "orden", "estabilidad",
        "patria", "nación", "república", "democracia",
    ],
    "disgusto": [
        "disgusto", "repugnancia", "repudio", "rechazo", "vergüenza", "deshonra",
        "escándalo", "corrupto", "depravado", "inmoral", "vicioso", "degradante",
    ],
    "anticipacion": [
        "espera", "futuro", "porvenir", "próximo", "venidero", "próximamente",
        "anuncio", "inauguración", "apertura", "proyecto", "plan", "propuesta",
    ],
}
...
_PALABRAS_SUBJETIVAS = {
    # verbos de opinión
    "creer", "opinar", "pensar", "considerar", "estimar", "juzgar", "suponer",
    "afirmar", "sostener", "argumentar", "defender", "criticar", "elogiar",
    # adjetivos evaluativos
    "bueno", "malo", "excelente", "pésimo", "notable", "destacado", "ilustre",
    "digno", "indigno", "loable", "reprobable", "admirable", "deplorable",
    "brillante", "mediocre", "extraordinario", "lamentable",
    # adverbios de modalidad
    "desafortunadamente", "lamentablemente", "afortunadamente", "evidentemente",
    "indudablemente", "naturalmente", "ciertamente", "sinceramente",
}

_PALABRAS_FACTUALES = {
    # verbos de reporte
    "informar", "reportar", "publicar", "anunciar", "declarar", "señalar",
    "indicar", "mostrar", "revelar", "confirmar", "negar",
    # expresiones de dato
    "según", "datos", "estadísticas", "cifras", "número", "total",
    "porcentaje", "fecha", "lugar", "año", "hora",
}
...
def analizar_emociones(texto: str) -> dict:
    ...
    if not texto:
        return {
            "emociones": {e: {"n": 0, "porcentaje": 0.0}
                          for e in _LEXICON_EMOCIONES},
    ...
    conteo_emo: dict[str, int] = {e: 0 for e in _LEXICON_EMOCIONES}
    ...
    subjetividad = round(
        n_subjetivas / (n_subjetivas + n_factuales + 1), 3
    )
...
def analizar_subjetividad(texto: str) -> dict:
    ...
    """
    if not texto:
        return {"score_subjetividad": 0.0, "tipo_discurso": "factual",
                "marcadores_subjetivos": [], "marcadores_factuales": [],
                "oraciones_subjetivas": [], "oraciones_factuales": []}
...
def analizar_intensidad(texto: str) -> dict:
    ...
    if superlativos:
        marcadores.extend({"tipo": "superlativo", "ejemplo": s}
                          for s in superlativos[:3])
    ...
    _CUANT = {"nunca", "siempre", "jamás", "todo", "nada", "nadie", "todos",
               "completamente", "absolutamente", "totalmente", "enormemente"}
    cuant = [p for p in _re.findall(r'\b\w+\b', texto.lower()) if p in _CUANT]
    if cuant:
        marcadores.extend({"tipo": "cuantificador", "ejemplo": c}
                          for c in sorted(set(cuant))[:3])
