        ...
        for ent in sent.ents:
            if ent.label_ in ("PER", "PERSON"):
                ents_per.append({
                    ...
                    "start":     ent.start,
                    "end":       ent.end,
                    "menciones": [{"texto": ent.text, "tipo": "entidad",
                                   "sent_idx": i}],
                })
    ...
    try:
        for chain in doc._.coref_chains:
            ...
            for mention in chain:
                ...
                menciones.append({
                    "texto": texto_m,
                    "tipo":  "entidad" if any(t.ent_type_ for t in toks)
                             else "pronombre",
                })
            if menciones:
                cadenas.append({
...
def estadisticas_coref(
    ...
    return {
        ...
        "promedio_menciones":       round(suma_menciones / total_cadenas, 2)
                                    if total_cadenas > 0 else 0,
