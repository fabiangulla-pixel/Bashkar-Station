...
_STOPWORDS_ES = {
    ...
    "por", "con", "del", "se", "le", "su", "al", "no", "lo", "más", "pero",
    "este", "esta", "ha", "para", "como", "sus", "muy", "también", "fue",
    "ser", "está", "son", "si", "ya", "todo", "hay", "sin", "sobre", "entre",
    "cuando", "gran", "hasta", "donde", "bien", "así", "porque", "después",
    "mismo", "cada", "vez", "tan", "dos", "antes", "años", "año", "Colombia",
    "colombiano", "colombiana", "revista", "estampa",
}
...
def etiquetar_topicos_llm(resultado: dict, api_key: str) -> dict:
    ...
    topicos_str = "\n".join(
        f"Tópico {tid}: {', '.join(info['palabras'][:8])}"
...
def modelar_topicos(
    ...
    if usar_bertopic:
        try:
            ...
            resultado = _modelar_nmf(textos_validos, n_topicos, callback,
                                     min_df=min_df, max_df=max_df, n_palabras=n_palabras)
    else:
        resultado = _modelar_nmf(textos_validos, n_topicos, callback,
                                 min_df=min_df, max_df=max_df, n_palabras=n_palabras)
...
def exportar_topicos_csv(resultado: dict, ruta: Path) -> int:
    ...
    with open(ruta, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=["articulo_idx", "topico_id",
                                           "topico_nombre", "palabras_clave"])
        w.writeheader()
        w.writerows(filas)
    return len(filas)
