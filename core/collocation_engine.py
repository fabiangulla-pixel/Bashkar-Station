...
def _tokenizar(texto: str, stopwords: bool = True,
               lematizar: bool = False, nlp=None) -> list[str]:
    """
...
def red_lexica(
    ...
    nodos = [
        {"id": w, "label": w, "freq": freq_total[w],
         "size": min(40, 10 + freq_total[w] // 5)}
        for w in nodos_sel if w in freq_total
    ]
...
def frecuencias(
    ...
    return [
        {"palabra": w, "freq": f, "df": df[w]}
        for w, f in freq_global.most_common(top_n)
    ]
...
def ngramas(
    ...
    return [
        {"ngrama": ng, "frecuencia": f}
        for ng, f in contador.most_common(top_n)
        if f >= min_freq
    ]
