...
def calcular_similitud_corpus(
    ...
    textos = [
        articulos[a].get("texto_limpio") or articulos[a].get("texto_ocr") or " "
        for a in ids
    ]
...
def analizar_intertextualidad(
    ...
    if usar_llm and api_key and pares:
        ...
        for par in pares[:max_pares_llm]:
            ...
            conn = detectar_intertextualidad_llm(
                texto_a, texto_b, titulo_a, titulo_b, api_key
            )
            if conn.get("tipo") != "sin_conexion":
                conexiones_llm.append({**par, **conn})

    _log(f"Listo. {len(citas)} citas compartidas, {len(pares)} pares similares, {len(conexiones_llm)} conexiones LLM.")
    return {
